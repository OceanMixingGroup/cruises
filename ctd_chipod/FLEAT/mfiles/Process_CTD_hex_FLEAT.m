...
cruisepath=fullfile('/Users/Andy/Cruises_Research/OceanMixingGroup/cruises','ctd_chipod')

addpath(fullfile( mixpath, 'ctd_processing' ) )
addpath(fullfile( cruisepath, project, 'mfiles' ))

% *** For recording filename used to process data
this_file_name='Process_CTD_hex_FLEAT.m'
...
for icast=1:length(ctdlist)
    ...
    try
        ...
        % in this data, con files are uppercase for some reason?
        clear full_upper
        full_upper=fullfile(CTD_data_dir,upper(ctdlist(icast).name));
        confile=[full_upper(1:end-3) 'XMLCON']
