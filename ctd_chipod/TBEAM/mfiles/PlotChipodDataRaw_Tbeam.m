%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%
% PlotChipodDataRaw_Tbeam.m
%
% Plot the raw chipod data files, to do a quick check for any issues with
% data.
%
% * something wrong with sn1001 times?
%
%-----------------------------
% 07/05/16 - A. Pickering - apickering@coas.oregonstate.edu - Modified from
% template
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%%
...
for iSN=2:length(allSNs)
    ...
    chi_file_list=dir( fullfile(data_dir,['/*' ChiInfo.(whSN).suffix '*']))
...
%                figname=fullfile(figdir,[whSN '_RawData_' chi_file_list(whfile).name(1:end-4)])
                figname=fullfile(figdir,[whSN '_RawData_' chi_file_list(whfile).name]);
